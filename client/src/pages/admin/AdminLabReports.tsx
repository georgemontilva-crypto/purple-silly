...
function ActionButton({
  ...
  onClick?: () => void;
  href?: string;
  tone?: "muted" | "vivid" | "pink";
}) {
  const [hover, setHover] = useState(false);
  const color = tone === "vivid" ? C.vivid : tone === "pink" ? C.pink : C.muted;
  const style: React.CSSProperties = {
    padding: "0.3rem 0.75rem", borderRadius: "0.4rem", fontSize: "0.75rem",
    border: `1px solid ${hover ? color : alpha(color, 45)}`,
    background: hover ? alpha(color, 15) : "transparent",
    color, cursor: "pointer", fontWeight: 600, textDecoration: "none",
    ...
    <div>
      ...
      <form onSubmit={handleUpload} style={{
        ...
        <div style={{ marginBottom: "1rem" }}>
          ...
          <div
            ...
            style={{
              border: `2px dashed ${file ? C.vivid : alpha(C.border, 35)}`,
              borderRadius: "0.75rem",
              padding: "1.5rem",
              textAlign: "center",
              cursor: "pointer",
              background: file ? alpha(C.vivid, 10) : "transparent",
          ...
          >
            ...
            {file ? (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.4rem", color: C.vivid, fontWeight: 700 }}>
                <FileText size={16} /> {file.name} ({(file.size / 1024).toFixed(0)} KB)
              </div>
            ) : (
        ...
        <button type="submit" disabled={uploading || createReport.isPending}
          ...
          style={{
            ...
            cursor: "pointer",
            opacity: (uploading || createReport.isPending) ? 0.6 : submitHover ? 0.85 : 1,
      ...
      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "1.25rem" }}>
        <button onClick={() => setFilterCat(0)} style={{
          ...
          border: `1px solid ${filterCat === 0 ? C.vivid : alpha(C.border, 35)}`,
          background: filterCat === 0 ? alpha(C.vivid, 22) : "transparent",
          color: filterCat === 0 ? C.bright : C.muted, cursor: "pointer",
          transition: "background 0.15s, border-color 0.15s, color 0.15s",
        }}>All</button>
        {categories?.map(c => (
          <button key={c.id} onClick={() => setFilterCat(c.id)} style={{
            padding: "0.35rem 0.9rem", borderRadius: "999px", fontSize: "0.8rem", fontWeight: 700,
            border: `1px solid ${filterCat === c.id ? C.vivid : alpha(C.border, 35)}`,
            background: filterCat === c.id ? alpha(C.vivid, 22) : "transparent",
            color: filterCat === c.id ? C.bright : C.muted, cursor: "pointer",
            transition: "background 0.15s, border-color 0.15s, color 0.15s",
          }}>{c.name}</button>
        ))}
      </div>
            ...
            <div key={r.id} style={{
              ...
              <div style={{ display: "flex", gap: "0.5rem", alignItems: "center", flexShrink: 0 }}>
                <span style={{
                  ...
                  background: r.isPublished ? alpha(C.vivid, 25) : alpha(C.pink, 25),
                  color: r.isPublished ? C.bright : C.pink,
                }}>
                  {r.isPublished ? "Published" : "Draft"}
                </span>
                <ActionButton href={r.fileUrl} tone="vivid">View PDF</ActionButton>
                <ActionButton onClick={() => togglePublish.mutate({ id: r.id, isPublished: r.isPublished ? 0 : 1 })}>
                  {r.isPublished ? "Unpublish" : "Publish"}
                </ActionButton>
                <ActionButton onClick={() => { if (confirm("Delete this report?")) deleteReport.mutate({ id: r.id }); }} tone="pink">
                  Delete
                </ActionButton>
              </div>
            </div>
